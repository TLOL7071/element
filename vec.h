...
void Vector<type>::pop()
{
    if (this->size >= 1)
    {
        ...
        if (size <= capacity / 4) {
            ...
            delete[]p;
            p = tempt; /*al:按照要求对利用capacity和size重写了，不过感觉没什么必要，如果你也认为这个缩小vector没必要的话，就删除这一段吧。(english version : According to the requirements,
...
{
    if(i>=size||i<0) throw(char)0;       //al:新增越界访问异常处理(english version:Add out-of-bounds access exception handling.)
    return this->p[i];
}
